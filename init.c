...
interrupt[TIM0_OVF] void timer0_ovf_isr() {
    ...
    if (is_show_clock) { 
        ...
        if (second != prev_sec) { 
            gui_print_time();
            // lam cai nay hoi ngu, nhung vi bo dem cua rtc va bo dem cua timer interrupt hoi lech nhau mot chut
            // nen neu dung interrupt de print time cua rtc thi sau mot khoang thoi gian (vai chuc giay) se bi lech 1 giay
            // (vi du tren dong ho se dem 1, 2, 3, 5, mat so 4 vi bi lech)
            // viec dem lech nay khong anh huong toi cac cai khac, chi la hien thi thoi, nhung kho chiu =))
        }
        prev_sec = second; 
        if (milisec % 500 == 0) {
            gui_colon_blink();
    ...
    for (i = 1; i <= 5; i++) {
        if ((node[i].active_hour[hour] == true) && (minute == 0)){
    		switch (i) {
				case 1:
		            relay_1                 =   ON;
		            node[1].status          =   ON;
		            node[1].alarm_time_s    =   1 * MIN_TO_SEC; //doi phut ra giay
	        	break;
		        case 2:
		            relay_2                 =   ON;
		            node[2].status          =   ON;
        ...
        all_relay_on = true;
        for (j = 1; j <= 5; j++) {
            if (node[i].status == OFF) {
                all_relay_on = false;
                break;
            }
        }
        if (all_relay_on) gps_send();
    }
